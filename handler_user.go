...
func (apiCfg *apiConfig) handlerGetUser(w http.ResponseWriter, r *http.Request, user sqlc.User) {
	respondWithJSON(w, 200, databaseUserToUser(user))
}

func (apiCfg *apiConfig) handlerGetPostsForUser(w http.ResponseWriter, r *http.Request, user sqlc.User) {
	posts, err := apiCfg.DB.GetPostsForUser(r.Context(), sqlc.GetPostsForUserParams{
		UserID: user.ID,
		Limit:  10,
	})
	if err != nil {
